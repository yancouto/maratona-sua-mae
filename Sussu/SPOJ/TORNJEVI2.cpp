...
char M[MAXN][MAXN];
int ind[MAXN][MAXN], mrk[MAXN*MAXN*4], c[MAXN*MAXN*4], degr[MAXN*MAXN*4];
int jah[MAXN*MAXN], jav[MAXN*MAXN], res[MAXN*MAXN];
int resv[MAXN], resh[MAXN], ja[MAXN*MAXN*4], jasetei[MAXN*MAXN*4];
int U[MAXN][MAXN]; 
int D[MAXN][MAXN]; 
int R[MAXN][MAXN];
int L[MAXN][MAXN]; 
...
vector<int> cadj[MAXN*MAXN*4], cinv[MAXN*MAXN*4];
int cdeg[MAXN*MAXN*4];
...
int main (){
	...
	/*{
		...
		*/
		...
		for(int a=sim(ver(1));a<=nao(hor(deg - 1));a++){
			if(degr[a] != 0) printf("%d de cor %d ta com deg %d\n", a, c[a], degr[a]);
		}
	}*/

	for(int a=1;a<=cor-1;a++){
		if(cdeg[a] == 0){
			topsrt.push(a);
		}
	}
	...
	while(!topsrt.empty()){
		int comp = topsrt.front();
		topsrt.pop();
		for(int cnxt: cadj[comp]){
			cdeg[cnxt]--;
			if(!cdeg[cnxt])
				topsrt.push(cnxt);
		}
		if(jasetei[comp])continue;
		seta(comp, false);
	}
