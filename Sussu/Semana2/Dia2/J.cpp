...
vector<int> adjP[MAXR], adjQ[MAXR];
int cycP[MAXR], cycQ[MAXR];
int tP = 1, tQ = 1;
int vis[MAXR], bom[MAXR];

int par[MAXR], ok[MAXR];
...
void dfsP(int v, int orig) {
	debug("DFS(%d, %d)\n", v, orig);
	cycP[v] = tP;
	for(int u : adjP[v])
		if(u != orig) dfsP(u, orig);
}

void dfsQ(int v, int orig) {
	debug("%d\n", v);
	cycQ[v] = tQ;
	for(int u : adjQ[v])
		if(u != orig) dfsQ(u, orig);
}
...
int main() {
	...
	for(int i = 0; i < r; i++) {
		for(int j = 0; j < r; j++) {
			if(cycP[i] == cycP[j]) join(i, j);
			if(cycQ[i] == cycQ[j]) join(i, j);
		}
	}
	
	for(int i = 0; i < r; i++) {
		if(ok[find(i)]) ans[find(i)]++;
	}
	
	BigInteger b(1);
	int cnt = ans.size();
	while(cnt--) b *= BigInteger(2);
	cout << string(b) << endl;
