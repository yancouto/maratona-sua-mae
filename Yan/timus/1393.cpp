...
int main() {
	...
	pot[0] = 1; pot2[0] = 1;
	for(i = 0; i < n; i++) {
		pot[i + 1] = mod(41ll * pot[i]);
		h[i + 1] = mod(41ll * h[i] + s[i%(n/2)] - 'A' + 1);
		pot2[i + 1] = 37u * pot2[i];
		h2[i + 1] = 37u * h2[i] + s[i%(n/2)] - 'A' + 1;
	}
	for(i = 0; i < n; i++)
		l[i] = no(s[i], 0, i);
	sort(l, l + n);
	for(i = 0; i < n; i++) {
		p[l[i].i][0] = i && l[i] == l[i-1]? p[l[i-1].i][0] : i;
	}
	for(j = 1; ; j++) {
		int sz = 0;
		for(i = 0; i < n; i++) {
			l[i] = no(pp(i, j-1), i + (1 << (j - 1)) < n? pp(i + (1 << (j - 1)), j - 1) : -1, i);
			if(l[i].p.snd == -1) l[sz++] = l[i];
			else g[c[l[i].p.snd]++ + l[i].p.snd] = l[i];
		}
		for(i = 0; i < n; i++)
			while(c[i])
				l[sz++] = g[i + --c[i]];
		for(i = n-1; i >= 0; i--)
			g[c[l[i].p.fst]++ + l[i].p.fst] = l[i];
		sz = 0;
		for(i = 0; i < n; i++)
			while(c[i])
				l[sz++] = g[i + --c[i]];
